...
pub fn main() !void {
    ...
    inline for (0..3) |_| {
        ...
        const avg_us: f64 = @as(f64, @floatFromInt(avg_ns)) / @as(f64, @floatFromInt(std.time.ns_per_us));

        const totsal_insert: usize = n / 1000;

        std.debug.print(
            "{d}k insert ops: {d:.3} us total, {d:.2} us per op\n",
            .{ totsal_insert, total_us, avg_us },
        );
    }
}
